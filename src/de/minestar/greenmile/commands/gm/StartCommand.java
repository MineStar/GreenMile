...
public class StartCommand extends ExtendedCommand {
    ...
    public void execute(String[] args, CommandSender sender) {
        ...
        if (!this.worldManager.worldExists(worldName)) {
            ChatUtils.printError(sender, this.pluginName, " Welt '" + worldName + "' nicht gefunden!");
            return;
        }

        if (Main.chunkThread != null) {
            ChatUtils.printError(sender, this.pluginName, "Es läuft bereits ein Erzeugungsthread!");
            ChatUtils.printInfo(sender, this.pluginName, ChatColor.GRAY, "Es läuft bereits ein Erzeugungsthread!");
            return;
        }
        ...
        ChatUtils.printSuccess(sender, this.pluginName, "Die Welt '" + worldName + "' wird nun mit einer Geschwindigkeit von " + pSpeed + " erzeugt!");
        ChatUtils.printInfo(sender, this.pluginName, ChatColor.GRAY, "'/gm stop' hält den Thread an!");
    }
}