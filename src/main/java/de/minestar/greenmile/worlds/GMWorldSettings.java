...
public class GMWorldSettings {
	 ...
	 * @param dataFolder
	 */
	public GMWorldSettings(String worldName, long levelSeed,
			World.Environment environment, File dataFolder) {
	...
	public boolean saveSettings(String worldName, File dataFolder) {
		...
		try {
			...
			ConsoleUtils.printException(e, GreenMileCore.NAME,
					"Can't save settings for world " + worldName + "!");
			return false;
		}
		return true;
	}
	...
	private boolean loadMainSettings(String worldName, File dataFolder) {
		try {
			...
			// GET VARS
			World.Environment environment = EnumHelper.getEnvironment(config
					.getString("settings.environment",
			...
			ConsoleUtils.printInfo(GreenMileCore.NAME, "Worldsettings for '"
					+ worldName + "' loaded!"); // \nlevelSeed =
												// " + this.levelSeed + "\nEnvironment
												// = " +
			...
			ConsoleUtils.printException(e, GreenMileCore.NAME,
					"Can't load worldsettings for world " + worldName + "!");
		}
		return false;
	}
	...
	public boolean loadWorldSettings(String worldName, File dataFolder) {
		try {
			...
			// GET VARS
			Difficulty difficulty = EnumHelper.getDifficulty(config.getString(
					"settings.difficulty", this.difficulty.toString()));

			if (difficulty == null) {
				throw new Exception("Difficulty not found!");
			}

			setSpawnMonsters(config.getBoolean("settings.spawnMonsters",
					this.spawnMonsters));
			setSpawnAnimals(config.getBoolean("settings.spawnAnimals",
					this.spawnAnimals));
			setAutoSave(config.getBoolean("settings.autoSave", this.autoSave));
			setKeepSpawnLoaded(config.getBoolean("settings.keepSpawnLoaded",
					this.keepSpawnLoaded));
			setDifficulty(difficulty);
			setMaxSize(config.getInt("settings.maxSize", this.maxSize));
			setWorldSpawn(new Location(Bukkit.getWorld(worldName),
					config.getDouble("settings.spawn.x", 0.0D),
					config.getDouble("settings.spawn.y", 128.0D),
					config.getDouble("settings.spawn.z", 0.0D),
					(float) config.getDouble("settings.spawn.yaw", 0.0D),
					(float) config.getDouble("settings.spawn.pitch", 0.0D)));

			String lastR = config.getString("lastRenderedChunk.x");
			if (lastR != null) {
				if (!lastR.equalsIgnoreCase("NULL")) {
					setLastRenderedPosition(new Point(
							config.getInt("lastRenderedChunk.x"),
							config.getInt("lastRenderedChunk.y")));
				}
			}

			ConsoleUtils.printInfo(GreenMileCore.NAME,
					"Specific settings for '" + worldName + "' loaded!"); // \nSpawnMonster
																			// =
																			// " + this.spawnMonsters + "\nSpawnAnimals
																			// =
																			// " + this.spawnAnimals + "\nAutoSave
			...
			ConsoleUtils.printException(e, GreenMileCore.NAME,
					"Can't load worldspecific settings for world " + worldName
							+ "!");
		}
		return false;
	}
