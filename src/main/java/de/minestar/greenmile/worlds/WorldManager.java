...
public class WorldManager {
	...
	private void addWorld(GMWorld world) {
		if (worldExists(world.getWorldName())) {
			ConsoleUtils.printWarning(GreenMileCore.NAME,
					"World '" + world.getWorldName() + "' already exists!");
			return;
		}
	...
	public boolean createWorld(String worldName, World.Environment environment,
			long seed, File dataFolder) {
		if (worldExists(worldName)) {
			return false;
		}
		GMWorld newWorld = new GMWorld(worldName, dataFolder);
		boolean result = GMWorld.loadOrCreateBukkitWorld(worldName,
				environment, seed);
		if (result) {
	...
	public boolean importWorld(WorldData worldData) {
		...
		newWorld.createSettings(worldData.getSeed(),
				World.Environment.getEnvironment(worldData.getType().g()),
				this.dataFolder);
		newWorld.getWorldSettings().setWorldSpawn(
				Bukkit.getWorld(worldData.getName()).getSpawnLocation());
		newWorld.getWorldSettings().saveSettings(worldData.getName(),
				dataFolder);
		...
		try {
			...
			for (File f : files) {
				...
				if ((fileName.endsWith(".yml"))
					...
					if (GMWorld.loadOrCreateBukkitWorld(worldName, world
							.getWorldSettings().getEnvironment(), world
							.getWorldSettings().getLevelSeed())) {
						// ADD WORLD TO WORLDLIST
						addWorld(world);
						// LOAD WORLDSPECIFIC SETTINGS
						world.getWorldSettings().loadWorldSettings(worldName,
								this.getDataFolder());
						// UPDATE BUKKIT WORLD
			...
			for (World world : Bukkit.getWorlds()) {
				if (!this.worldExists(world.getName())) {
					...
					WorldData worldData;
					if (!levelFile.exists()
							|| (worldData = this.getWorldData(levelFile)) == null) {
						continue;
					}

					if (!this.importWorld(worldData))
						ConsoleUtils.printError(GreenMileCore.NAME,
								"Could not import world '" + world.getName()
										+ "'!");
					else
						ConsoleUtils.printWarning(GreenMileCore.NAME, "World '"
								+ world.getName() + "' imported!");

				}
			}
		} catch (Exception e) {
			ConsoleUtils.printException(e, GreenMileCore.NAME,
					"Can't load worlds from " + this.dataFolder);
		}
	}
	...
	public WorldData getWorldData(File levelFile) {
		...
		if (levelFile.exists()) {
			try {
				mainCompound = NBTCompressedStreamTools
						.a((InputStream) (new FileInputStream(levelFile)));
